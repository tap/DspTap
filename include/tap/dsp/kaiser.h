/// @file kaiser.h
/// @brief Kaiser-window FIR prototype design for polyphase filter banks.
// SPDX-License-Identifier: MIT
// Copyright 2026 Timothy Place and the DspTap contributors.
//
// Carried from SampleRateTap (include/srt/detail/kaiser.h), where it designs
// the ASRC's interpolation bank; promoted here so RatioTap's fixed-ratio
// tables and any future FIR design share one implementation. All methods are
// published literature: Kaiser window design (Kaiser 1974), band-limited
// interpolation (J. O. Smith, CCRMA), the harris length estimate, and the
// zeros-at-k*fs droop-compensated variant worked out in the June 2026
// music-dsp thread (verified in SampleRateTap's asrc_rbj_analysis notebook).
//
// ANCHOR: kai_design_note
/// Design note — runtime vs constexpr: prototype tables run 12K-33K taps and
/// each tap needs sin/sqrt plus a ~50-term Bessel I0 series. Constexpr
/// evaluation is interpreted (roughly 1e3-1e4x slower than native), would need
/// hand-rolled constexpr transcendentals before C++26, and would cost tens of
/// seconds to minutes of compile time in every including translation unit.
/// Runtime design takes well under 10 ms, runs once in a constructor, and is
/// off the audio path, so all design math here is plain runtime double
/// precision.
...
namespace tap::dsp {
    ...
    /// suggestion, done inside the passband spec — see SampleRateTap's epilogue
    /// chapter and its asrc_rbj_analysis notebook).
    ///
