...
// shipped coefficients, the integer group delay, and the Q15 / Q31 profiles
// tracking the double golden model within each format's own floor. The
// float/double contract tests are typed over both profiles.
...
namespace {
    ...
    // (half a float ulp at the signal's level); pinned at 2x.
