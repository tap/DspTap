...
namespace {
    ...
    TEST(SineAnalysis, FloatingSpansAreBitIdenticalToThePreTemplateInstrument) {
        const double       nu = 997.0 / 48000.0;
    ...
    TEST(SineAnalysis, Q15SpanAgreesWithTheDoubleInstrumentToTheQuantisationNoise) {
        quantised_sine_agrees<std::int16_t>(0.2886 / 2.0, 0.2886 * 2.0, 92.07 - 0.5, 1.69e-3 * 2.0, 2.60e-3 * 2.0);
    }

    TEST(SineAnalysis, Q31SpanAgreesWithTheDoubleInstrumentToTheQuantisationNoise) {
    ...
    TEST(MultitoneAnalysis, TypedTailsMatchTheDoubleInstrumentAndReachTheirQuantisationFloor) {
        const an::tone_comb comb = an::tone_comb::pink(12, 40.0, 20000.0, 0.5);
        std::vector<double> exact(32768);
        for (std::size_t i = 0; i < exact.size(); ++i) {
            exact[i] = comb.sample_at(i, 48000.0);
        }
        const std::vector<float> tail32(exact.begin(), exact.end());
        ...
        // The exponent scales the tail, not the ratio.
        EXPECT_NEAR(an::program_weighted_snr_db(q15, comb, 48000.0, 48000.0, 3), snr_q15, 1e-9);
    }
