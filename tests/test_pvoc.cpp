...
namespace {
    ...
    TYPED_TEST(pvoc_test, IdentityRatioReconstructsTheWaveform) {
        ...
        tap::dsp::basic_pvoc<TypeParam> shifter(1024);
        const auto                      out = run_sine(shifter, 440.0, 1.0, 1.0, k_sr);
        ...
        for (size_t t = out.size() - 4800; t < out.size(); ++t) {
            const double expected = std::sin(2.0 * std::numbers::pi * 440.0 * static_cast<double>(t - latency) / k_sr);
    ...
    TYPED_TEST(pvoc_test, ShiftAccuracyAcrossRatioRange) {
        for (const double ratio : {0.5, 0.8, 1.122462, 1.5, 2.0}) {
            tap::dsp::basic_pvoc<TypeParam> shifter(1024);
            const auto                      out = run_sine(shifter, 220.0, ratio, 1.0, k_sr);
            EXPECT_LT(std::abs(cents(measure_hz(out, k_sr), 220.0 * ratio)), 8.0) << "ratio " << ratio;
    ...
    TYPED_TEST(pvoc_test, OutputStaysFinite) {
        ...
        for (int t = 0; t < 48000; ++t) {
            ...
            const double x = 0.4 * std::sin(2.0 * std::numbers::pi * 110.0 * t / k_sr)
                             + 0.3 * std::sin(2.0 * std::numbers::pi * 347.0 * t / k_sr)
                             + 0.3 * std::sin(2.0 * std::numbers::pi * 991.0 * t / k_sr);
    ...
    std::vector<Sample> formant_source(double f0, double center_hz, size_t n) {
        ...
        for (int h = 1; h <= 20; ++h) {
            ...
            for (size_t t = 0; t < n; ++t) {
                x[t] += static_cast<Sample>(amp * std::sin(2.0 * std::numbers::pi * f * static_cast<double>(t) / k_sr));
            }
        }
        return x;
    }
    ...
    double band_energy(const std::vector<Sample>& x, double lo_hz, double hi_hz) {
        ...
        for (size_t i = 0; i < n; ++i) {
            const double w = tap::dsp::detail::periodic_hann(i, n);
    ...
    TYPED_TEST(pvoc_test, ClearZerosTheState) {
        tap::dsp::basic_pvoc<TypeParam> shifter(1024);
        run_sine(shifter, 440.0, 1.5, 0.25, k_sr);
        shifter.clear();
    ...
    TYPED_TEST(pvoc_test, ClockWrapIsBitExactAndCountersDoNotOverflow) {
        ...
        for (int t = 0; t < warm + run; ++t) {
            ...
            const TypeParam x = static_cast<TypeParam>(std::sin(2.0 * std::numbers::pi * 440.0 * t / k_sr));
            ...
            if (t >= 2 * latency) { // steady state: four frames overlap from here on
                const double expected = std::sin(2.0 * std::numbers::pi * 440.0 * (t - latency) / k_sr);
    ...
    TEST(pvoc_cross_precision, FloatAgreesWithDoubleGoldenModel) {
        ...
        const auto       od = run_sine(gold, 220.0, 1.5, 1.0, k_sr);
        const auto       of = run_sine(fast, 220.0, 1.5, 1.0, k_sr);
