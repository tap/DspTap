...
cited as "audit Part N" below; it lands with the plan PR, tap/DspTap#25, which
merges before this note, so the link is a forward link only until then), and
each PR links its stage.*

## Purpose and scope

`tap::dsp::basic_real_fft<Sample>` (`include/tap/dsp/fft.h`) is today a
two-profile real-FFT contract — `double` (the golden model) and `float` (the
embedded profile, optionally accelerated by vDSP or CMSIS-Helium); the plan's
Stage 3 extends the same contract to the Q15 / Q31 fixed-point profiles from
`sample_traits.h`. The contract is the packing, the sign convention, the
scaling, and the numbers in the table below; the engine behind it changes
(vendored C today, a C++20 port of the same split-radix algorithm next, a
radix-4 int32 kernel for fixed point) and the contract does not. Consumers hold
...
cross-reference, and an entry that names a test is pinned by it. Entries that
name only a source (an audit probe, a measurement) are not yet pinned and say
which stage pins them. Floating-point rows are the shipping `fft.h` at
`5ca3b1c`; fixed-point rows are the Part 7 design and fill in at Stage 3b.

| Contract point | `double` (`real_fft`) | `float` (`real_fft32`) | Q15 (`real_fft_q15`, planned) | Q31 (`real_fft_q31`, planned) |
|---|---|---|---|---|
...
- Headroom: the 4√2 per-stage bound, the packed-pair pattern at a 45°
  twiddle, and why two guard bits suffice for Q15 under fixed scaling.
- Twiddle quantization: `max |w_q − w| ≤ 0.5 LSB` of Q1.30 and its
  contribution to the floor.
- The Welch noise model per profile and policy: the predicted per-bin floor
  vs input level at N = 256 / 512 / 2048 (`TODO(stage 3b)`, derivation), and
  the comparison against the measured floors for white noise and an on-bin
  tone from the executed `notebooks/fft.ipynb` (`TODO(stage 3c)`, numbers).
